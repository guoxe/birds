...
X = zeros(16*length(training_data(:)), feature_size*channels); %gradient mag + grayscale
labels = zeros(16,1);
labels(1:4) = 1;
...
for i=1:length(training_data(:));
    ...
    im = im2double(im);
    pos_boxes = zeros(4,feature_size*channels);%3 channels, gradient mag, grayscale and max-min
    neg_boxes = zeros(12,feature_size*channels);%3 channels, gradient mag, grayscale and max-min
    ...
    for  j=1:4; 
        ...
        max_min = rangefilt(reshape(c2, 40, 40));
        max_min = reshape(max_min, 1, feature_size);
        pos_boxes(j,1:1600) = c1;
        pos_boxes(j,1601:3200) = c2;
        pos_boxes(j, 3201: 4800) = max_min;
    end;
    for j=1:12;
